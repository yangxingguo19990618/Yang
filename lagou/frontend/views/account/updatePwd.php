...
<!-- end #header -->
    ...
    <dl id="user_sideBarmenu" class="user_sideBarmenu">
     	        <dt><h3>帐号设置</h3></dt>
                <dd><a href="<?=Url::toRoute(['account/index'])?>">帐号绑定</a></dd>
        <dd><a class="hover" href="<?=Url::toRoute(['account/password'])?>">修改密码</a></dd>
            </dl>
</div>
<input type="hidden" id="hasSidebar" value="1">	<div class="content user_modifyContent">
        <dl class="c_section">
            ...
            <dd>
                <?php if(!empty($now_account)) {?>
                <form id="updatePswForm">
            		<table class="savePassword">
            			<tbody><tr>
            				<td>登录邮箱</td>
            				<td class="c7"><?=$now_account['email']?></td>
            			</tr>
            			<tr>
                ...
                <?php }else {?>
                    <dd id="pad">
                        <ul class="user_noModify">
                            <li>您还登录，点击 <font size="18"><a href="http://yxg.lagou.com/index.php?r=login/login">登录</a></font>进行登录</li>
                        </ul>
                    </dd>
                <?php }?>
            </dd>
...
<script>
    ...
    $('#updatePswForm').validate({
        ...
        submitHandler:function(form){
            ...
            $.ajax({
                url:'<?=Url::toRoute(['account/password'])?>',
                type:'POST',
                data:{
                ...
                if(result.success){
                    ...
                    setCountdown(4,'updatePassword h4 span','<?=Url::toRoute(['index/index'])?>');	//调用倒计时
                }else{
